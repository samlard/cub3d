...
void    moove_left(t_data *data)
{
    ...
    new_y = data->player->pos_y - ((float) cos(deg_to_rad(data->player->pa)) * MOVE_SPEED* data->speed_factor);
    new_x = data->player->pos_x - ((float) sin(deg_to_rad(data->player->pa)) * MOVE_SPEED* data->speed_factor);
    if (new_y >= data->nbr_line * 64 || new_y <= 0 || new_x >= data->larg_row * 64 || new_x <= 0)
        return;
    if (data->map[(int) (new_y /(float)SQUARE_SIZE)][(int) (new_x /(float)SQUARE_SIZE)] == '1')
        return;
